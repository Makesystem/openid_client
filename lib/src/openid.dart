...
class Issuer {
  ...
  Issuer(this.metadata, {this.claimsMap = const {}}) : _keyStore = metadata.jwksUri == null ? JsonWebKeyStore() : (JsonWebKeyStore()..addKeySetUrl(metadata.jwksUri!));
...
class Client {
  ...
  Credential createCredential({String? accessToken, String? tokenType, String? refreshToken, Duration? expiresIn, DateTime? expiresAt, String? idToken}) => Credential._(
      this,
...
class Credential {
  ...
  Future<void> revoke() async {
    ...
    var request = _token.refreshToken != null ? {'token': _token.refreshToken, 'token_type_hint': 'refresh_token'} : {'token': _token.accessToken, 'token_type_hint': 'access_token'};

    if (methods.contains('client_secret_basic')) {
      var h = base64.encode('${client.clientId}:${client.clientSecret ?? ''}'.codeUnits);
      await http.post(client.issuer.tokenEndpoint, headers: {'authorization': 'Basic $h'}, body: request, client: client.httpClient);
    } else {
      await http.post(uri, body: {...request, 'client_id': client.clientId, if (client.clientSecret != null) 'client_secret': client.clientSecret}, client: client.httpClient);
    }
  }
  ...
  Uri? generateLogoutUrl({Uri? redirectUri, String? state}) {
    return client.issuer.metadata.endSessionEndpoint?.replace(
        queryParameters: {'id_token_hint': _token.idToken.toCompactSerialization(), if (redirectUri != null) 'post_logout_redirect_uri': redirectUri.toString(), if (state != null) 'state': state});
  }

  http.Client createHttpClient([http.Client? baseClient]) => http.AuthorizedClient(baseClient ?? client.httpClient ?? http.Client(), this);
  ...
  Stream<Exception> validateToken({bool validateClaims = true, bool validateExpiry = true}) async* {
    ...
    yield* Stream.fromIterable(idToken.claims
        .validate(expiryTolerance: const Duration(seconds: 30), issuer: client.issuer.metadata.issuer, clientId: client.clientId, nonce: nonce)
  ...
  Future<TokenResponse> getTokenResponse([bool forceRefresh = false]) async {
    if (!forceRefresh && _token.accessToken != null && (_token.expiresAt == null || _token.expiresAt!.isAfter(DateTime.now()))) {
      return _token;
    }
...
class Flow {
  ...
  Flow._(this.type, this.responseType, this.client,
      {String? state, String? codeVerifier, Map<String, String>? additionalParameters, Uri? redirectUri, List<String> scopes = const ['openid', 'profile', 'email']})
    ...
    var challenge = base64Url.encode(SHA256Digest().process(Uint8List.fromList(verifier.codeUnits))).replaceAll('=', '');
  ...
  Flow.authorizationCode(Client client,
      {String? state, String? prompt, String? accessType, Uri? redirectUri, Map<String, String>? additionalParameters, List<String> scopes = const ['openid', 'profile', 'email']})
      : this._(FlowType.authorizationCode, 'code', client,
            state: state,
            additionalParameters: {if (prompt != null) 'prompt': prompt, if (accessType != null) 'access_type': accessType, ...?additionalParameters},
            scopes: scopes,
  ...
  Flow.authorizationCodeWithPKCE(
    ...
    Map<String, String>? additionalParameters,
          ...
          additionalParameters: {if (prompt != null) 'prompt': prompt, ...?additionalParameters},
        );
  ...
  Uri get authenticationUri => client.issuer.metadata.authorizationEndpoint.replace(queryParameters: _authenticationUriParameters);
  ...
  Map<String, String?> get _authenticationUriParameters {
    var v = {..._additionalParameters, 'response_type': responseType, 'scope': scopes.join(' '), 'client_id': client.clientId, 'redirect_uri': redirectUri.toString(), 'state': state}
  ...
  Future<TokenResponse> _getToken(String? code) async {
    ...
    if (type == FlowType.jwtBearer) {
      ...
      json = await http.post(client.issuer.tokenEndpoint,
          body: {'grant_type': 'authorization_code', 'code': code, 'redirect_uri': redirectUri.toString(), 'client_id': client.clientId, 'client_secret': client.clientSecret},
          client: client.httpClient);
    } else if (methods.contains('client_secret_basic')) {
      var h = base64.encode('${client.clientId}:${client.clientSecret}'.codeUnits);
      json = await http.post(client.issuer.tokenEndpoint,
          headers: {'authorization': 'Basic $h'}, body: {'grant_type': 'authorization_code', 'code': code, 'redirect_uri': redirectUri.toString()}, client: client.httpClient);
    } else {
  ...
  Future<Credential> callback(Map<String, String> response) async {
    ...
    } else if (response.containsKey('code') && (type == FlowType.proofKeyForCodeExchange || client.clientSecret != null)) {
...
class OpenIdException implements Exception {
  ...
  static const _defaultMessages = {
    ...
    'unauthorized': 'The Client is not authorized to use the User Questioning API or did not send a valid Access Token.',
  ...
  /// the issuer metadata
  const OpenIdException.missingTokenEndpoint() : this._('missing_token_endpoint', 'The issuer metadata does not contain a token endpoint.');
